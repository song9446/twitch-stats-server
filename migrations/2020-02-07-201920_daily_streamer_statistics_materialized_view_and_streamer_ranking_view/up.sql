...
CREATE TABLE viewer_migration_counts (
    source BIGINT NOT NULL REFERENCES streamers (id),
    destination BIGINT NOT NULL REFERENCES streamers (id),
    migration_count INT NOT NULL DEFAULT 0,
    time TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    PRIMARY KEY (source, destination, time)
);

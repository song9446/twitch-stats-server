<script>
  ...
  function on_input_change(e) {
    ...
    if(streamers.length){
      ...
      fetch("/api/thin-streamers?search=" + input_value)
        .then(res=>res.json())
        .then(streamers=>{
          filter(streamers, keyword);
        });
...
<div class="inline-block relative" >
  <input 
    ...
    class="px-2 border rounded focus:outline-none w-48 text-xs leading-loose pr-6"
  ...
  <ul class="absolute bg-white" class:hidden="{!focused}">
    {#each filtered_streamers as streamer}
      <li on:mousedown={e=>on_list_click(streamer)} class="w-48 px-2 py-2 hover:bg-purple-500 hover:text-white text-black flex flex-row items-center truncate">
